...
$password = 'VMware1!'
Export-ModuleMember -Variable vcuser
...
Export-ModuleMember -Variable maxMinutesBeforeFail
...
$LabCheckInterval = 1
Export-ModuleMember -Variable LabCheckInterval

#Set the start time
Set-Variable -Name startTime -Value $(Get-Date) -Scope Global
Export-ModuleMember -Variable startTime

$LabCheck = $false
Export-ModuleMember -Variable LabCheck

# sleep time between checks, in seconds
$sleepSeconds = 10
Export-ModuleMember -Variable sleepSeconds

#IP information - define global variables and set initial values
Set-Variable -Name YEAR -Value 111 -Scope Global
Set-Variable -Name SKU -Value 222 -Scope Global
Set-Variable -Name IPNET -Value '192.168.250' -Scope Global
...
$resourceFileDir = Join-Path $labStartupRoot 'Resources'
...
Export-ModuleMember -Variable readyTimeFile

#### Define new PS Types ####

# Bypass SSL certificate verification (not for production use!)
Add-Type @'
	using System.Net;
...
# Type used to detect intereractive use (LabCheck kill switch)
Add-Type @'
using System;
...
##############################################################################
# Status codes used by Report-VpodStatus
##############################################################################
$statusTable = @{
	'GOOD-1'   = 1
	'GOOD-2'   = 2
...
Export-ModuleMember -Function Start-AutoLab
...
#>
	If( Test-Path $plinkPath ) {
		...
		Write-Error "UNABLE TO FIND $plinkPath"
		Return $false
	}
} #End Invoke-Plink
Export-ModuleMember -Function Invoke-Plink
...
Export-ModuleMember -Function Invoke-PlinkKey
...
#>
	If( Test-Path $pscpPath ) {
		...
		Write-Error "UNABLE TO FIND $pscpPath"
		Return $false
	}
} #End Invoke-Pscp
Export-ModuleMember -Function Invoke-Pscp
...
#>
	...
	If( -Not (Test-Path $psexecPath) ) {
		Write-Error "UNABLE TO FIND $psexecPath"
		Return $false
	} Else {
...
Export-ModuleMember -Function Report-VpodStatus
...
#>
	...
	If ( $code.Contains("FAIL") ) { 
		#any FAIL changes text color to Red (3A3AFA) in desktopInfo
...
Export-ModuleMember -Function Write-VpodProgress
...
Export-ModuleMember -Function Get-CloudInfo
...
Export-ModuleMember -Function Connect-Restart-vCenter
	...
	ForEach ($record in $records) {
		...
		If ( $vcenter -eq $null ) {
			If( $vCenters.Count -gt 1 ) {
				($vcenter,$type) = $vCenters[0].Split(":")
			} Else {
				($vcenter,$type) = $vCenters.Split(":")			
			}
			Write-Output "Using default vCenter: $vCenter for vVM/vApp $name"
		}
		ElseIf ( $name -eq "Pause" ) {
...
Export-ModuleMember -Function Start-Nested
...
Export-ModuleMember -Function Connect-Restart-HVCS
...
Export-ModuleMember -Function ManageLinuxService
...
Export-ModuleMember -Function StartWindowsServices
...
Export-ModuleMember -Function ManageWindowsService
...
Export-ModuleMember -Function ManageVcsaService
...
Export-ModuleMember -Function Get-RuntimeSeconds
...
Export-ModuleMember -Function LabStartup-Sleep
...
Export-ModuleMember -Function LabFail
...
Export-ModuleMember -Function Check-Datastore
...
Export-ModuleMember -Function Get-URL
...
} # End Create-LabCheckTask
Export-ModuleMember -Function Create-LabCheckTask
...
Function CleanFirefoxAnnoyFile {
<#
	Remove the file that causes the "Reset" message in Firefox
#>
    Write-Output "$(Get-Date) Cleaning Firefox Reset trigger file"
...
} #END CleanFirefoxAnnoyFile
Export-ModuleMember -Function CleanFirefoxAnnoyFile

Function ResetModuleSwitcherState {
<#
	Clean up HOL ModuleSwitcher state file(s)
#>
    Write-Output "$(Get-Date) Resetting ModuleSwitcher State"
...
} #END ResetModuleSwitcherState
Export-ModuleMember -Function ResetModuleSwitcherState


Function Test-LabCheck {
<# 
	determine if this is first run or a labcheck run
#>
	PARAM ( [string]$scriptArg0) 
	PROCESS {
		If( $scriptArg0 -eq 'labcheck' ) { 
			$labcheck = $true
			# check for screen resolution change from default and disable LabCheck task
			[void][Reflection.Assembly]::LoadWithPartialName("System.Windows.Forms")
			$ScreenSize = [System.Windows.Forms.SystemInformation]::PrimaryMonitorSize
			If( ($ScreenSize.Width -ne $screenWidth) -Or ( $ScreenSize.Height -ne $screenHeight ) ) {
				Write-Host "ScreenSize: " $ScreenSize.Width " X " $ScreenSize.Height
				Write-Host "Detected screen resolution change. Lab is active! LabCheck will no longer run."
				$result = UnRegister-ScheduledTask -TaskName "LabCheck" -Confirm:$false -ErrorAction SilentlyContinue
				Exit
			}
			# if input has occurred after the last run, then disable the task and exit
			$LastInput = [PInvoke.Win32.UserInput]::LastInput
			$Now = (Get-Date).ToUniversalTime()
			$LastRun = $Now.AddHours(-$labCheckInterval)
			If ($LastInput -gt $LastRun) { 
				Write-Host "Detected input since last run. Lab is active! LabCheck will no longer run."
				$result = UnRegister-ScheduledTask -TaskName "LabCheck" -Confirm:$false -ErrorAction SilentlyContinue
				Exit
			}
			$coldStartMin = Get-Content ($readyTimeFile)
        } ElseIf( $scriptArg0 -eq 'labchecktest') {
            $labcheck = $true
			$coldStartMin = Get-Content ($readyTimeFile)
		} Else { $labcheck = $false }
...
Export-ModuleMember -Function Test-LabCheck


Function Parse-LabSKU {
<# 
	determine whether the SKU has been properly set
#>
	PARAM ( [string]$vPodSKU ) 

	PROCESS {
	
		If( $vPodSKU -eq 'HOL-BADSKU' ) {
			# Unconfigured SKU. Use the default IP network and fail the pod.
		...
		Catch {
			# Problem found. Use the default IP network and fail the pod.
			Write-Output "ERROR - malformed lab SKU: $TMP"
			Write-VpodProgress "FAIL-Bad Lab SKU" 'FAIL-1'
			Break
		}
		
		Write-Output "`tvPod SKU configured: $vPodSKU... using network $IPNET.x"
	}
} #END Parse-LabSKU
Export-ModuleMember -Function Parse-LabSKU


Function Test-PowerCLI {
<#
	Checks for VMware.VimAutomation.Core module - PowerCLI 6.5.1 or higher
#>
	if( !( Test-Path 'C:\Program Files\WindowsPowerShell\Modules\VMware.VimAutomation.Core') ) {
		...
		Write-Output "`tPowerCLI found... good."
	}
} #END Test-PowerCLI
